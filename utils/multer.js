...
let productStorage = diskStorage({
    ...
    filename: (req, file, cb) => {
        ...
        cb(null, `${file.fieldname}-${Date.now()}-${req.body.price}-${req.body.name}.png`);
    },
});

const uploadCompanyFile = multer({ storage: companyStorage, fileFilter: imageFilter });
export const uploadProductFile = multer({ storage: productStorage, fileFilter: imageFilter });
